...
| モデル名 (Code) | 種類 | ステータス | 概要 |
| :--- | :--- | :--- | :--- |
| **wolf** | 画像 | ✅ 推奨 | 軽量・高速なタグベース判定 (V1) |
| **eagle** | 画像 | ✅ 推奨 | 高精度なTransformerベース判定 (V2) |
| **parfait** | テキスト | ✅ 推奨 | 高度なマルチラベル毒性検出 (V2) |
| **pancake** | テキスト | ⛔ 廃止 | **[非推奨]** 旧バージョンのモデル |

- **ハイブリッド認証**:
  - **APIキー**: バックエンド/サーバー間通信用。
  - **OAuth2**: ユーザーログイン（ダッシュボード）。
...
**URLパラメータ:**
- `model_name`: `pancake`, `parfait` など (モデルのコードネーム)

**使用例 (cURL):**

```bash
curl -X POST "https://puresight.kokonatsu.top/api/predict/text/pancake" \
...
req = urllib.request.Request(
    "https://puresight.kokonatsu.top/api/predict/text/pancake",
...
> [!NOTE]
> **`parfait` (V2) モデルの場合**
> `parfait` モデルでは、`details` フィールドに詳細な毒性カテゴリごとのスコアが含まれます。
> ```json
> {
>   "score": 0.98,
>   "text_classification_result": "Toxic",
...
**URLパラメータ:**
- `model_name`: `wolf` または `eagle`

**使用例 (cURL):**

```bash
curl -X POST "https://puresight.kokonatsu.top/api/predict/picture/eagle" \
  -H "X-API-Key: YOUR_API_KEY" \
...
req = urllib.request.Request(
    "https://puresight.kokonatsu.top/api/predict/picture/eagle",
    data=body,
...
> [!NOTE]
> **`wolf` (V1) モデルの場合**
> `wolf` モデルでは、`details` フィールドに追加の詳細判定スコアが含まれます。
